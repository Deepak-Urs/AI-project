import time
import copy
import heapq as heap # for tree nodes queuing with a priority
...
def calculate_main_search_algo(begin_state, end_state, class_search):
	...
	while (priority_queue):
		...
		if (equality_state(present_node_state, end_state)):
			print('A solution detected! The nodes and their expansion order is as follows: \n')
			present_node.display_nodes_traced()
			...
			for stateofnewchild in updated_collection_child:
				...
				if (priority_queue and child_new in priority_queue):
					...
					# Case 2: Manhattan Distance heuristic with A*
						...
						child_new.h = calculate_misplaced_tiles(child_new.curr_state, end_state)  
		...
		for j in range(0, len(begin_state)):
			for k in range(0, len(begin_state)):
				...
				if (i == end_state[j][k]):
					end_row = j
					end_col = k
		heuristic += (abs(end_col - begin_col) + abs(end_row - begin_row))

	return heuristic


def calculate_misplaced_tiles(begin_state, end_state):
	heuristic = 0
...
class Node:
	...
	def display_nodes_traced(self):
        ...
        # -1 done from result as it counts the beginning as well
		print('The count of nodes traced (aka Depth) : ', trace-1, '\n')
	...
	def tile_operators(self):
		...
		if (p != 0):
			...
			if (self.parent and equality_state(newstate, self.parent.curr_state)):
